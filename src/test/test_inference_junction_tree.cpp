...
//        network->LoadBIFFile(net_file);
        ...
        // version, we need to do the changes in `LoadLIBSVMTestingData`.
...
//        tester->LoadLIBSVMTestingData(test_set_file, 0);
        ...
        inference = new JunctionTree(false, network, tester);
//        SAFE_DELETE(network);
        SAFE_DELETE(tester);
    }
...
TEST_F(TestExactInferenceJTAlarm, thread_1) {
    string pt_file = dataset_path + "alarm/alarm_1k_pt";
    int num_threads = 1;
