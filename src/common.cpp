...
Parameter::Parameter() {
    ...
    net_file_path = "../dataset/alarm/alarm.xml"; // for inference
    test_set_file_path = "../dataset/alarm/testing_alarm_1k_p20"; // for inference
    prob_tab_file_path = "../dataset/alarm/alarm_1k_pt"; // for inference
    train_set_file_path = "../dataset/alarm/alarm_s5000.txt"; // for structure learning
    ref_net_path = "../dataset/alarm/alarm.bif"; // for structure learning
}

void Parameter::read_input_parameters(int argc, char *argv[]) {
    int i;
    for(i = 1; (i < argc) && ((argv[i])[0] == '-'); i++){
        switch((argv[i][1]))
        {
            ...
            case 's': i++; num_samples = atoi(argv[i]); break;
            case 'u': i++; num_updating = atoi(argv[i]); break;
            case 'g': i++; group_size = atoi(argv[i]); break;
            case 'f':
                switch (argv[i][2]) {
                    case '0': i++; net_file_path = argv[i]; break;
                    case '1': i++; test_set_file_path = argv[i]; break;
                    case '2': i++; prob_tab_file_path = argv[i]; break;
                    case '3': i++; train_set_file_path = argv[i]; break;
                    case '4': i++; ref_net_path = argv[i]; break;
                }; break;
    ...
    if(algorithm < 0 || algorithm > 9){
        cout << "must give the right value of a, 0 for pc-stable" << endl
             << "0 for PC-stable, 1 for brute force, 2 for junction tree, 3 for variable elimination, "
                "4 for probabilistic logic sampling, 5 for likelihood weighting, 6 for EPIS-BN, "
                "7 for loopy belief propagation, 8 for self-importance sampling, 9 for AIS-BN" << endl;
        exit(0);
    }
