  ...
  for (auto &xpp : vec_xml_elems_ptr) { // Parse each "PROBABILITY" element.
    ...
    for (int i = 0; i < nary_counts.size(); ++i) {
      ...
      if (digits.size() == 1) {
        ...
        // todo: check if "comb" equals to "empty_config" when the node has no parents.
        DiscreteConfig empty_config;
        dynamic_cast<DiscreteNode*>(for_np)->AddCount(query, empty_config, vec_db_table_entry.at(i)*10000);
      } else {
